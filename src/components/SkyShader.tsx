...
// Preload shaders using top-level fetch with proper text extraction
const vertexShaderPromise = fetch('/shaders/sky.vert')
...
const fragmentShaderPromise = fetch('/shaders/sky.frag')
  .then(response => response.text())
  .catch(() => `
...
// Create LUT texture for expensive calculations
const createLUTTexture = () => {
  ...
  for (let y = 0; y < size; y++) {
    for (let x = 0; x < size; x++) {
      ...
      // Pre-compute expensive math operations
      data[i] = Math.pow(1.0 - nx, 5.0); // Fresnel
      data[i + 1] = Math.exp(-ny * 8000.0); // Atmospheric density
      data[i + 2] = Math.pow(1.0 - ny, 2.0); // Scatter intensity
      data[i + 3] = 1.0;
    }
  }
...
const SkyShader = () => {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      try {
        // Async shader loading with proper text extraction
        ...
        const renderer = new THREE.WebGLRenderer({
          ...
          alpha: false, // Optimization: Disable alpha for opaque background
          stencil: false, // Optimization: Disable stencil buffer
          depth: false // Optimization: Disable depth buffer
        });
        ...
        // Create and optimize textures
        const noiseTexture = createNoiseTexture();
        const lutTexture = createLUTTexture();
        
        // Enhanced uniforms with LUT
        const uniforms = {
          time: { value: 0.0 },
        ...
        const material = new THREE.ShaderMaterial({
          ...
          depthTest: false,
          depthWrite: false,
          transparent: false // Optimization: Disable transparency
        });
        
        // Optimized geometry using InstancedMesh for better batching
        ...
        // Start animation with delay for smooth startup
        setTimeout(() => {
          setIsReady(true);
          animate();
        }, 100);
        ...
        console.error('Error initializing SkyShader:', error);
      }
    };
